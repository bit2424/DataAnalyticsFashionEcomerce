...
print(pcolors.OKGREEN + pcolors.UNDERLINE + pcolors.BOLD + "<<<---------------------------- ORIGINAL ----------------------------\n" + pcolors.ENDC + pcolors.ENDC)
print(df.columns)
print("\n")
print(df.shape)
...
"""Acerca de las varables que no podemos controlar, hay varias que se correlacionan bastante con la cantidad vendida, cómo lo son el rating_count y  los rating_x_count, pero dado que están por fuera de nuestro control, buscamos una manera de realacionaar esas variables con los tags."""

df_interesting = df.loc[:,interesting_features]
print(pcolors.OKGREEN + pcolors.UNDERLINE + pcolors.BOLD + "<<<---------------------------- FEATURES OF INTEREST ----------------------------\n" + pcolors.ENDC + pcolors.ENDC)
...
"""Asignamos valores coherentes para remplazar los nands encontrados en el dataset."""
...
print(pcolors.OKGREEN + pcolors.UNDERLINE + pcolors.BOLD + "<<<---------------------------- SOME CLEANING WAS MADE ----------------------------\n" + pcolors.ENDC + pcolors.ENDC)
print(df_interesting.info())
print(pcolors.OKGREEN + pcolors.UNDERLINE + pcolors.BOLD + "   ---------------------------- SOME CLEANING WAS MADE ---------------------------->>>\n\n" + pcolors.ENDC)
...
print(pcolors.OKGREEN + pcolors.UNDERLINE + pcolors.BOLD + "<<<---------------------------- CORRELATION ----------------------------\n" + pcolors.ENDC + pcolors.ENDC)
...
A partir de las variables que tenemos, intentaremos hacer ajustes para ayudar al modelo.

Identifiquemos los colores que más venden y realizemos una clasificación en donde pongamos a los colores menos relevantes en una catégoria propia, eliminando ruido que puede afectar la predicción.
"""
...
print(pcolors.OKGREEN + pcolors.UNDERLINE + pcolors.BOLD + "<<<---------------------------- ONE HOT ENCODING ----------------------------\n" + pcolors.ENDC + pcolors.ENDC)
...
"""En esta parte agrgaremos 2 columnas que se relacionan con los tags, los tags son un atributo importante ya que encapsulan mucha información acerca del producto:


*   best_tag_units_sold: Escogemos el valor del tag con las mejores ventas promedio de ese producto
*   Elemento de lista: Calculamos el promedio del promedio de las ventas de todos los tag de ese producto
...
print(pcolors.OKGREEN + pcolors.UNDERLINE + pcolors.BOLD + "<<<---------------------------- TAGS DICTIONARY ----------------------------\n" + pcolors.ENDC + pcolors.ENDC)
print(dic_tags_uts)
print(pcolors.OKGREEN + pcolors.UNDERLINE + pcolors.BOLD + "   ---------------------------- TAGS DICTIONARY ---------------------------->>>\n\n" + pcolors.ENDC)
...
print(pcolors.OKGREEN + pcolors.UNDERLINE + pcolors.BOLD + "<<<---------------------------- AVG PRICE TAGS DICTIONARY ----------------------------\n" + pcolors.ENDC + pcolors.ENDC)
print(dic_tags_uts)
print(pcolors.OKGREEN + pcolors.UNDERLINE + pcolors.BOLD + "   ---------------------------- AVG PRICE TAGS DICTIONARY ---------------------------->>>\n\n" + pcolors.ENDC)
...
print(pcolors.OKGREEN + pcolors.UNDERLINE + pcolors.BOLD + "<<<---------------------------- APPENDED COLUMNS TO DATAFRAME ----------------------------\n" + pcolors.ENDC + pcolors.ENDC)
print(df_interesting.info())
print(pcolors.OKGREEN + pcolors.UNDERLINE + pcolors.BOLD + "   ---------------------------- APPENDED COLUMNS TO DATAFRAME ---------------------------->>>\n\n" + pcolors.ENDC)
